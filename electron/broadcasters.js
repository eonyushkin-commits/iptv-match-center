 ...
 * одной и той же тысячи имён. Замерено: 980 → 116 мс.
 */
...
/**
 * Все `tvg-id`, чьё имя совпадает с именем станции. ВСЕ, а не первый
 * попавшийся: одно имя станции часто накрывает несколько разных id — как
 * правило это варианты качества одного канала («Match! Football 1» → Матч!
 * Футбол 1 FHD / HD / HD Double / SD), на живых данных так у 58 заявок из
 * 321. Здесь стоял `find()`, и тогда выбор качества определялся порядком
 * каналов в плейлисте: у текущего провайдера они перечислены по убыванию,
 * так что случайно бралось лучшее — но логики за этим не было, и другой
 * плейлист молча отдавал бы SD или зеркало Double. Заодно это ровно то же
 * поведение, что у поиска по заголовкам EPG: тот всегда возвращает все
 * каналы, на которых нашёл матч.
 *
 * Совпадение требуется ТОЧНОЕ (`similarityTokens() === 100`) — именно оно,
 * а не что-либо ещё, не пускает сюда потоковые сервисы вроде «DAZN.com (FR)»
 * и «Sky Sports+ app», которых в ответе больше трети.
 */
