 ...
 *          structured clone переносит Map через границу потока как есть, с
 *          ключами любого типа. Здесь было написано, что иначе числовые id
 *          матчей превратились бы в строки — это неверно, id у FotMob и так
 *          строковые (проверено у источника), и объект сработал бы тоже.
 *          Map остаётся ради независимости от этого: смени источник тип —
 *          объект молча привёл бы ключи обратно к строкам, и sync.js не нашёл
 *          бы ни одного матча, не выдав ни единой ошибки.
 */
