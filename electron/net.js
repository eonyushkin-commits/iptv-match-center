...
// Столько запросов к FotMob держим в воздухе разом. Синк дёргает под сорок
// турниров, столько же стран и ещё по запросу на каждый живой матч — залпом
// это удар по недокументированному хосту, ровно того класса, на котором в
// этом проекте уже обжигались (безлимитный Promise.all к Wikidata молча
// возвращал 0 результатов из 393). Строго по одному тоже плохо: сорок стран
// подряд занимали 47 секунд чистого накопления круговых задержек при 16
// секундах на весь остальной синк.
//
// Живёт здесь, а не у каждого вызывающего: это свойство хоста, а не задачи.
// Раньше значение стояло тремя копиями с комментариями «та же величина, что
// в соседнем файле, по той же причине» — и одна из трёх копий всё равно
// отстала (fotmob.js уточнял минуту живых матчей безлимитно).
const BATCH_SIZE = 8;
...
module.exports = { fetchJson, BATCH_SIZE };
