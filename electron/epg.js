...
function programmes(xmlSource, channels, kickoffs = []) {
  ...
  let pos = 0;
  // Разбор 200-мегабайтного буфера — ~850 мс сплошной работы. Функция
  // намеренно СИНХРОННАЯ и чистая: планированию в её сигнатуре не место. За
  // то, чтобы это не морозило интерфейс, отвечает граница процессов — весь
  // этап целиком выполняется в рабочем потоке (electron/sync-worker.js).
  //
  // Ускорять тут нечего, профилировано: горячей точки нет — 110 мс на поиск
  // границ тегов, 60 на срезы в строки, остальное размазано по 424 тысячам
  // передач. Предкомпиляция регулярок в attr() экономит 20 мс из 855, а
  // «очевидно быстрый» разбор даты по позициям вместо регулярки оказался
  // вдвое медленнее.
  for (;;) {
